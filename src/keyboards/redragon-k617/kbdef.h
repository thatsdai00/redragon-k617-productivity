...
enum custom_keycodes {
    ...
    RESET_HOLD,
    FACT_RESET,
    SNAP_TAP_TOGGLE, /* Fn+Q: A/D Snap Tap on/off */
    WIN_LOCK_TOGGLE, /* Fn+Win: block Windows key */
    RGB_SOFT_CYCLE,  /* Fn+N: color softness 1..5 */
    KB_SAFE_RANGE,
};

// Matrix probe: each electrical cell reports its own (row,col). Handled in kb.c.
// Base kept clear of the RGB/reset custom keycodes above (SAFE_RANGE..).
#define PROBE_BASE 0x7F00u
#define PROBE_KEYCODE(r, c) (uint16_t)(PROBE_BASE + ((r) * MATRIX_COLS) + (c))
...
// Key columns — NuPhy/BYK916 (v3/v4 working lights + base typing)
...
// Stock firmware (IDA @0x503E): Caps Lock LED = P7.7, active-low
#define LED_CAPS     P7_7
#define LED_CAPS_BIT _P7_7
