...
const uint8_t hid_report_desc_extra[] = {
    ...
    HID_RI_USAGE(8, 0x01),                // Vendor ISP
    HID_RI_COLLECTION(8, 0x01),           // Application
    ...
    /* WebHID config channel — Usage 0x02 (ISP stays Usage 0x01). */
    HID_RI_USAGE_PAGE(16, 0xff00),
    HID_RI_USAGE(8, 0x02),
    HID_RI_COLLECTION(8, 0x01),
        HID_RI_REPORT_ID(8, REPORT_ID_VENDOR),
        HID_RI_USAGE_MINIMUM(8, 0x01),
        HID_RI_USAGE_MAXIMUM(8, 0x07),
        HID_RI_LOGICAL_MINIMUM(8, 0x00),
        HID_RI_LOGICAL_MAXIMUM(16, 0x00ff),
        HID_RI_REPORT_SIZE(8, 8),
...
usb_ascii_string_c usb_strings[] = {
    ...
    [USB_STRING_PRODUCT - 1]       = "Redragon K617",
...
static void usb_hid_get_report_handler(__xdata struct usb_req_setup *req);
...
void usb_interrupt_handler() __interrupt(_INT_USB)
{
    ...
    if (temp_usbif1 != 0x00) {
        if (temp_usbif1 & _SOFIF) {
            USBIF1 &= ~_SOFIF;
            /* Host is sending frames → bus is alive. Clears false SUSPIF lock. */
            indicators_wake();
        } else {
            ...
            if (temp_usbif1 & _PUPIF) {
                ...
                USBIF1 &= ~_SUSPIF;
                /* PC sleep / shutdown: blank RGB (SOF will wake on resume) */
                indicators_suspend();
...
static void usb_hid_get_report_handler(__xdata struct usb_req_setup *req)
{
    ...
    uint8_t id   = (uint8_t)(req->wValue & 0xff);

    if (type == REPORT_TYPE_FEATURE && id == REPORT_ID_VENDOR) {
        webhid_fill_status(EP0_IN_BUF);
        SET_EP0_CNT(VENDOR_REPORT_SIZE);
        SET_EP0_IN_RDY;
        usb_ep0_state = USB_EP0_STATE_RECV_STATUS;
        return;
    }
    ...
    switch (req->wValue >> 8) {
        ...
        case REPORT_TYPE_FEATURE:
            if ((req->wValue & 0xff) == REPORT_ID_ISP) {
                ...
                SET_EP0_OUT_RDY;
            } else if ((req->wValue & 0xff) == REPORT_ID_VENDOR) {
                usb_ep0_state = USB_EP0_STATE_VENDOR;
                SET_EP0_OUT_RDY;
            }
...
void usb_ep0_out_irq()
{
    ...
    } else if (usb_ep0_state == USB_EP0_STATE_VENDOR) {
        usb_ep0_state = 0;
        webhid_handle_set(EP0_OUT_BUF, VENDOR_REPORT_SIZE);
        CLEAR_EP0_CNT;
        SET_EP0_IN_RDY;
    } else {
        usb_ep0_state = 0;
