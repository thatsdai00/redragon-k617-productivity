...
enum report_id {
    ...
    REPORT_ID_VENDOR   = 3, /* WebHID config — Usage Page 0xFF00 / Usage 0x02 */
    REPORT_ID_ISP      = 5,
    REPORT_ID_NKRO     = 6,
    REPORT_ID_CONSOLE  = 7,
};

/* Feature report: Report ID + 7 payload bytes (matches HID descriptor count). */
#define VENDOR_REPORT_SIZE 8
#define VENDOR_MAGIC       0x4B /* 'K' */
#define VENDOR_PROTO_VER   1

enum vendor_cmd {
    VCMD_SET_EFFECT     = 0x01,
    VCMD_SET_BRIGHTNESS = 0x02,
    VCMD_SET_SPEED      = 0x03,
    VCMD_SET_SOFTNESS   = 0x04,
    VCMD_SET_SNAPTAP    = 0x05,
    VCMD_SET_WINLOCK    = 0x06,
    VCMD_NEXT_EFFECT    = 0x07,
    VCMD_SAVE           = 0x08,
    VCMD_SET_SNAPMODE   = 0x09, /* unused; reserved */
    VCMD_SET_IDLE       = 0x0A, /* 0=30s .. 3=240s, 4=off */
};
